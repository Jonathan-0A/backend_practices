...
export const addMember = asyncHandler(async (req, res) => {
    try {
        ...
        const lastMember = await Member.findOne().sort({ serial_id: -1 }).select("serial_id");
        req.body.serial_id = lastMember ? lastMember.serial_id + 1 : 1;
        ...
        try {
            const sanitizedData = sanitizeData(memberMapper([member]));
